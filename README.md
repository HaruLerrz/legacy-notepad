...
- 貼上剪貼簿內容時一律以純文字插入，避免 RichEdit 帶入來源格式。
- 改善尋找與取代對話框的鍵盤操作：開啟後自動聚焦尋找欄位，支援 `Ctrl+F` / `Ctrl+H` 切換、輸入欄位 `Ctrl+A` 全選，以及 `Tab` / `Shift+Tab` 導覽。
- 修正單次取代讀取選取範圍的方式，改用 RichEdit 原生 `EM_GETSELTEXT`；「全部取代」找不到符合項目時會顯示既有提示訊息。
- 保留原專案程式版本號；公開預覽版以 fork 專用 release tag 標示。
